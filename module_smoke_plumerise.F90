...
! RAR:
  ...
  real(RKIND),  INTENT(IN) ::  lat,long, curr_secs, alpha ! SRB

  REAL(RKIND), INTENT(IN) :: frp_min 

!  integer, intent(in) :: PLUMERISE_flag
  real(kind=RKIND) :: frp_inst   ! This is the instantenous FRP, at a given time step
  real(kind=RKIND) :: g, cp, rgas, cpor
  ...
  real(kind=RKIND),    dimension(m1,m2,m3) :: up, vp, wp,theta,pp,dn0,rv 
  real(kind=RKIND),    dimension(m1)       :: zt_rams,zm_rams
  real(kind=RKIND)                         :: burnt_area,dzi,FRP   ! RAR:
  real(kind=RKIND),    dimension(2)        :: ztopmax
  real(kind=RKIND)                         :: q_smold_kgm2
   ...
   !frp_inst = max(1000.,frp_inst)
    ...
    lp_minmax: do imm=1,2
       ...
       IF (frp_inst<frp_min) THEN
         ...
         return
       END IF
       !write(*,*),'In plumerise, frp_inst > frp_min,calling get fire properties'
       ...
       if(errflg/=0) return
       !write(*,*),'back from fire prop,burnt_area = ',burnt_area
       !------  generates the plume rise    ------
       call makeplume (coms,kmt,ztopmax(imm),ixx,imm,alpha)

    enddo lp_minmax
        !write(*,*),'calling set_flam_vert, ztopmax,coms%zzcon(5) = ',ztopmax,coms%zzcon(5)
...
real(kind=RKIND),parameter :: p1000mb = 100000.  ! p at 1000mb (pascals)
real(kind=RKIND),parameter :: p00=p1000mb
real(kind=RKIND) :: znz,themax,tlll,plll,rlll,zlll,dzdd,dzlll,tlcl,plcl,dzlcl,dummy
!integer :: n_setgrid = 0 
integer :: wind_eff
character(*), intent(inout) :: errmsg
integer, intent(inout) :: errflg
...
!-------------------------------------------------------------------------
  ...
  SUBROUTINE set_flam_vert(ztopmax,k1,k2,nkp,zzcon) !,W_VMD,VMD)
    ...
    REAL(kind=RKIND)    , INTENT(IN)  :: ztopmax(2)
    INTEGER , INTENT(OUT) :: k1,k2

    ! plumegen_coms
    INTEGER , INTENT(IN)  :: nkp
    REAL(kind=RKIND)    , INTENT(IN)  :: zzcon(nkp)

    INTEGER imm,k
    INTEGER, DIMENSION(2)  :: k_lim

    !- version 2
!    REAL(kind=RKIND)    , INTENT(IN)  :: W_VMD(nkp,2)
!    REAL(kind=RKIND)    , INTENT(OUT) ::   VMD(nkp,2)
!    real(kind=RKIND)   w_thresold,xxx
...
real(kind=RKIND)::   bfract,  effload,  heat,  hinc ,burnt_area,heat_fluxW,FRP
!real(kind=RKIND),    dimension(2,4) :: heat_flux
integer, intent(inout) :: errflg
character(*), intent(inout) :: errmsg
INTEGER, parameter :: use_last = 1    ! RAR 10/31/2022: I set to one, checking with Saulo

!real(kind=RKIND), parameter :: beta = 5.0   !ref.: Wooster et al., 2005
REAL(kind=RKIND), parameter :: beta = 0.88  !ref.: Paugam et al., 2015
...
!-------------------------------------------------------------------------------
!
SUBROUTINE MAKEPLUME (coms,kmt,ztopmax,ixx,imm,alpha)
!
! *********************************************************************
!
...
! Set threshold to be the same as dz=100., the constant grid spacing of plume grid model(meters) found in set_grid()
    REAL(kind=RKIND) :: DELZ_THRESOLD = 100. 

    INTEGER     :: imm, dtknt

!  real(kind=RKIND), external:: esat_pr!
!
! ******************* SOME CONSTANTS **********************************
!
...
implicit none
type(plumegen_coms), pointer :: coms
!real(kind=RKIND), parameter :: HEAT = 21.E6 !Joules/kg
!real(kind=RKIND), parameter :: HEAT = 15.5E6 !Joules/kg - cerrado
real(kind=RKIND), parameter :: HEAT = 19.3E6 !Joules/kg - floresta em Alta Floresta (MT)
real(kind=RKIND) :: eflux,water
!
...
implicit none
type(plumegen_coms), pointer :: coms
real(kind=RKIND), parameter :: g = 9.80796, r = 287.04, cp = 1004.6, eps = 0.622,tmelt = 273.3
real(kind=RKIND), parameter :: tfreeze = 269.3, pi = 3.14159, e1 = 1./3., e2 = 5./3.
real(kind=RKIND) :: es,  esat, eflux, water,  pres, c1,  c2, f, zv,  denscor, xwater !,ESAT_PR
!  real(kind=RKIND), external:: esat_pr!
...
implicit none 
type(plumegen_coms), pointer :: coms
real(kind=RKIND), parameter :: tfreeze = 269.3
integer ::  isub,  k,  n1,  n2,  n3,  lbuoy,  itmp,  isubm1 ,kmt
real(kind=RKIND) ::     xn1,  xi,  es,  esat!,ESAT_PR
REAL(kind=RKIND)    , INTENT(IN)    :: alpha
!
COMS%N=kmt
...
real(kind=RKIND) dt
real(kind=RKIND), dimension(nm1) :: w,t,tt,qv,qh,qi,qc,te,pe,qvenv,dummy,zt,zm
...
implicit none
real(kind=RKIND), dimension(nm1) :: var1,var2,vart,zt,zm
integer k,nfpt,kf,nm1,ifrom,deltak
real(kind=RKIND) zmkf,ztop,distim,c1,c2,dt
...
implicit none
integer :: k,m1
real(kind=RKIND), dimension(m1) :: wc,wt,flxw,dzm,rho
real(kind=RKIND), dimension(m1) :: dn0 ! var local
real(kind=RKIND) :: c1z
...
integer :: k,iac
integer :: m1,mintime
real(kind=RKIND), dimension(m1) :: dummy, wc,wt,wp
real(kind=RKIND) eps,dt
...
implicit none
integer :: npts,iac,m
real(kind=RKIND) :: epsu,dtlp
real(kind=RKIND), dimension(*) :: ac,ap,fa,af
...
implicit none
integer :: k,m1
real(kind=RKIND), parameter :: g = 9.8, eps = 0.622, gama = 0.5 ! mass virtual coeff.
real(kind=RKIND), dimension(m1) :: T, TE, QV, QVENV, QH, QI, QC, WT, scr1
real(kind=RKIND) :: TV,TVE,QWTOTL,umgamai
real(kind=RKIND), parameter :: mu = 0.15 
...
type(plumegen_coms), pointer :: coms
integer :: k,m1
real(kind=RKIND), dimension(m1) :: w,wt,radius
REAL(kind=RKIND) DMDTM,WBAR,RADIUS_BAR,umgamai,DYN_ENTR,ALPHA
real(kind=RKIND), parameter :: mu = 0.15 ,gama = 0.5 ! mass virtual coeff.
...
implicit none
integer :: m1,k
real(kind=RKIND) :: dfact
real(kind=RKIND), dimension(m1) :: scp,scr1,vt3dc,vt3df,vt3dg,vt3dk
real(kind=RKIND), dimension(m1) :: vctr1,vctr2
...
implicit none
integer :: m1,k
real(kind=RKIND) :: dtl,dtli
real(kind=RKIND), dimension(m1) :: scp,sca,sct
...
implicit none
type(plumegen_coms), pointer :: coms
real(kind=RKIND), parameter :: g = 9.81, cp=1004.
integer m1,k
real(kind=RKIND) dmdtm
REAL(kind=RKIND)    , INTENT(IN)    :: alpha
 do k=2,m1-1
...
!     ****************************************************************
  ...
  SUBROUTINE scl_dyn_entrain(m1,nkp,wbar,w,adiabat,alpha,radius,tt,t,te,qvt,qv,qvenv,qct,qc,qht,qh,qit,qi,&
    ...
    ! plumegen_coms
    INTEGER , INTENT(IN)    :: nkp
    REAL(kind=RKIND)    , INTENT(INOUT) :: wbar 
    REAL(kind=RKIND)    , INTENT(IN)    :: w(nkp)
    REAL(kind=RKIND)    , INTENT(INOUT) :: adiabat 
    REAL(kind=RKIND)    , INTENT(IN)    :: alpha
    REAL(kind=RKIND)    , INTENT(IN)    :: radius(nkp)
    REAL(kind=RKIND)    , INTENT(INOUT) :: tt(nkp)
    REAL(kind=RKIND)    , INTENT(IN)    :: t(nkp)
    REAL(kind=RKIND)    , INTENT(IN)    :: te(nkp)
    REAL(kind=RKIND)    , INTENT(INOUT) :: qvt(nkp)
    REAL(kind=RKIND)    , INTENT(IN)    :: qv(nkp)
    REAL(kind=RKIND)    , INTENT(IN)    :: qvenv(nkp)
    REAL(kind=RKIND)    , INTENT(INOUT) :: qct(nkp)
    REAL(kind=RKIND)    , INTENT(IN)    :: qc(nkp)
    REAL(kind=RKIND)    , INTENT(INOUT) :: qht(nkp)
    REAL(kind=RKIND)    , INTENT(IN)    :: qh(nkp)
    REAL(kind=RKIND)    , INTENT(INOUT) :: qit(nkp)
    REAL(kind=RKIND)    , INTENT(IN)    :: qi(nkp)

    REAL(kind=RKIND)    , INTENT(IN)    :: vel_e(nkp)
    REAL(kind=RKIND)    , INTENT(IN)    :: vel_p(nkp)
    REAL(kind=RKIND)    , INTENT(INOUT) :: vel_t(nkp)
    REAL(kind=RKIND)    , INTENT(INOUT) :: rad_T(nkp)
    REAL(kind=RKIND)    , INTENT(IN)    :: rad_p(nkp)

    real(kind=RKIND), parameter :: g = 9.81, cp=1004., pi=3.1416
...
real(kind=RKIND) dz1t,dz1m,dz2t,dz2m,d2wdz,d2tdz  ,d2qvdz ,d2qhdz ,d2qcdz ,d2qidz ,d2scdz, &
 d2vel_pdz,d2rad_dz
!sam real(kind=RKIND) :: old_tt
...
type(plumegen_coms), pointer :: coms
integer m1,k
real(kind=RKIND) vtc, dfhz,dfiz,dz1
!srf==================================
...
real(kind=RKIND), PARAMETER :: VCONST = 5.107387, EPS = 0.622, F0 = 0.75  
real(kind=RKIND), PARAMETER :: G = 9.81, CP = 1004.
!
do k=2,m1-1
...
!
real(kind=RKIND), PARAMETER :: HERC = 5.44E-4, CP = 1.004, HEATCOND = 2.5E3  
real(kind=RKIND), PARAMETER :: HEATSUBL = 2834., TMELT = 273., TFREEZE = 269.3

real(kind=RKIND), PARAMETER :: FRC = HEATCOND / CP, SRC = HEATSUBL / CP

real(kind=RKIND) :: evhdt, evidt, evrate, evap, sd,	quant, dividend, divisor, devidt
...
!use module_zero_plumegen_coms  
!
real(kind=RKIND),      PARAMETER ::  AK1 = 0.001    !conversion rate constant
real(kind=RKIND),      PARAMETER ::  AK2 = 0.0052   !collection (accretion) rate
real(kind=RKIND),      PARAMETER ::  TH  = 0.5      !Kessler threshold
integer,   PARAMETER ::  iconv = 1        !- Kessler conversion (=0)
                                     
!real(kind=RKIND), parameter :: ANBASE =  50.!*1.e+6 !Berry-number at cloud base #/m^3(maritime)
 real(kind=RKIND), parameter :: ANBASE =100000.!*1.e+6 !Berry-number at cloud base #/m^3(continental)
!real(kind=RKIND), parameter :: BDISP = 0.366       !Berry--size dispersion (maritime)
 real(kind=RKIND), parameter :: BDISP = 0.146       !Berry--size dispersion (continental)
real(kind=RKIND), parameter :: TFREEZE = 269.3  !ice formation temperature
!
real(kind=RKIND) ::   accrete, con, q, h, bc1,   bc2,  total
...
!use module_zero_plumegen_coms  
!
real(kind=RKIND), PARAMETER :: EPS = 0.622, HEATFUS = 334., HEATSUBL = 2834., CP = 1.004
real(kind=RKIND), PARAMETER :: SRC = HEATSUBL / CP, FRC = HEATFUS / CP, TMELT = 273.3
real(kind=RKIND), PARAMETER :: TFREEZE = 269.3

real(kind=RKIND) ::dtsubh,  dividend,divisor, subl
!
DTSUBH = 0.  
!
...
implicit none
type(plumegen_coms), pointer :: coms
real(kind=RKIND), PARAMETER :: HEATFUS = 334., CP = 1.004, EPS = 0.622, HEATSUBL = 2834.
real(kind=RKIND), PARAMETER :: FRC = HEATFUS / CP, FRS = HEATSUBL / CP, TFREEZE =  269.3
real(kind=RKIND), PARAMETER :: GLCONST = 0.025   !glaciation time constant, 1/sec
real(kind=RKIND) dfrzh
!
...
real(kind=RKIND), PARAMETER :: FRC = 332.27, TMELT = 273., F0 = 0.75   !ice velocity factor
real(kind=RKIND) DTMELT
!                                    
...
SUBROUTINE htint (nzz1, vctra, eleva, nzz2, vctrb, elevb, errmsg, errflg)
  ...
  REAL(kind=RKIND),    INTENT(IN ) :: vctra(nzz1)
  REAL(kind=RKIND),    INTENT(OUT) :: vctrb(nzz2)
  REAL(kind=RKIND),    INTENT(IN ) :: eleva(nzz1)
  REAL(kind=RKIND),    INTENT(IN ) :: elevb(nzz2)
  character(*), intent(inout) :: errmsg
  integer, intent(inout) :: errflg
  INTEGER :: l
  INTEGER :: k
...
!
real(kind=RKIND), PARAMETER :: CI1 = 6.1115, CI2 = 22.542, CI3 = 273.48
real(kind=RKIND), PARAMETER :: CW1 = 6.1121, CW2 = 18.729, CW3 = 257.87, CW4 = 227.3
real(kind=RKIND), PARAMETER :: TMELT = 273.3

real(kind=RKIND) ESAT_PR
real(kind=RKIND) temc , tem,esatm
!
...
subroutine plumerise_sofiev(kte, u_in, v_in, w_in, theta_in, pi_in, rho_in, qv_in, &
  ...
  real(RKIND), intent(in) :: u_in(kte), v_in(kte), w_in(kte)
  real(RKIND), intent(in) :: theta_in(kte), pi_in(kte), rho_in(kte), qv_in(kte)
  real(RKIND), intent(in) :: zmid(kte), z_lev(kte)
  ...
  ! Constants
  real(RKIND), parameter :: g0    = 9.81_RKIND
  real(RKIND), parameter :: Pf0   = 1.0e6_RKIND
  real(RKIND), parameter :: N0sq  = 2.5e-4_RKIND   ! per Sofiev et al. Eq (9): N0^2
  real(RKIND), parameter :: hp_min = 100.0_RKIND

  ! Sofiev coefs for ABL plumes
  real(RKIND), parameter :: a_abl = 0.24_RKIND, b_abl = 170.0_RKIND, g_abl = 0.35_RKIND, d_abl = 0.60_RKIND
  ! Sofiev coefs for free-tropo plumes
  real(RKIND), parameter :: a_ft  = 0.93_RKIND, b_ft  = 298.0_RKIND, g_ft  = 0.13_RKIND, d_ft  = 0.70_RKIND
  ! Sofiev coefs for trial injection
  real(RKIND), parameter :: a_det = 0.15_RKIND, b_det = 102.0_RKIND, g_det = 0.49_RKIND, d_det = 0.00_RKIND

  real(RKIND) :: Habl, N2_ft
  real(RKIND) :: Hp_trial, Hp_final
  ...
  ! Check for kpbl 
  if (.not. present(kpbl_in)) then
     ierr = 1
     return
  end if
  if (kpbl_in < 2 .or. kpbl_in > kte) then
     ierr = 2
     return
  end if

  ! Get the PBL height [m]
  Habl = max(0.0_RKIND, min(z_lev(kpbl_in), z_lev(kte)))

  ! Compute the Brunt-Vaisala Frequency in free-troposphere
  call compute_N2_ft(kte, theta_in, z_lev, kpbl_in, N2_ft)

  ! Step 1: trial height using Eq (21) in Sofiev et al. (2012)
  Hp_trial = sofiev_hp(Habl, frp_w, N2_ft, Pf0, N0sq, a_det, b_det, g_det, d_det)

  ! Step 2: compare to HABL; use Eq (13) if inside ABL, else Eq (19)
  if (Hp_trial <= Habl) then
     Hp_final = sofiev_hp(Habl, frp_w, N2_ft, Pf0, N0sq, a_abl, b_abl, g_abl, d_abl)
  else
     Hp_final = sofiev_hp(Habl, frp_w, N2_ft, Pf0, N0sq, a_ft,  b_ft,  g_ft,  d_ft)
  end if
  ...
  ! Sanity check
  if (k_max <= k_min) k_max = min(kte, k_min+1)

end subroutine plumerise_sofiev
...
subroutine plumerise_briggs(kte, u_in, v_in, w_in, theta_in, pi_in, rho_in, qv_in, &
  ...
  real(RKIND), parameter :: g0 = 9.81_RKIND
  real(RKIND), parameter :: hp_min = 100.0_RKIND
  real(RKIND), parameter :: U_min  = 0.1_RKIND
  real(RKIND), parameter :: dT_plume = 200.0_RKIND   ! pragmatic default

  integer :: kpbl
  real(RKIND) :: Habl, N2_ft, N_ft
  real(RKIND) :: U_pbl, rho_pbl, Ta, Tp
  real(RKIND) :: ustar, wstar, wrms
  ...
  kpbl = kpbl_in
  if (kpbl < 2 .or. kpbl > kte) then
     ierr = 2
     return
  end if

  cp_used = 1004.0_RKIND
  if (present(cp_in)) cp_used = cp_in

  ! Get the PBL height [m]
  Habl = max(0.0_RKIND, min(z_lev(kpbl), z_lev(kte)))

  ! Compute the Brunt-Vaisala Frequency in free-troposphere
  call compute_N2_ft(kte, theta_in, z_lev, kpbl, N2_ft)
  N_ft = sqrt(max(N2_ft, 1.0e-6_RKIND))

  ! Get PBL averaged windspeed and density
  call mean_pbl_met(kte, u_in, v_in, w_in, rho_in, kpbl, U_pbl, wrms, rho_pbl)
  U_pbl = max(U_pbl, U_min)
  rho_pbl = max(rho_pbl, 0.2_RKIND)

  ! Ambient temperature, taking first level temperature for now
  Ta = temperature_from_theta_pi(theta_in(1), pi_in(1), cp_used)

  ! Plume temperature, for now use a constant delta above ambience
  Tp = max(350.0_RKIND, Ta + dT_plume)

  ! Estimate the stability regime 
  call diagnose_regime(theta_in, z_lev, w_in, kpbl, N2_ft, regime)

  ! ---- B69 (Eq 17 in Sofiev et al. (2012), wildfire-adapted)
  if (regime == 1) then
     if (U_pbl <= 0.5_RKIND) then
        Hp69 = 5.7_RKIND * ( g0*frp_w / ( (N_ft**3) * Tp*cp_used*rho_pbl ) )**0.25_RKIND
     else
        Hp69 = 2.4_RKIND * ( g0*frp_w / ( N2_ft * U_pbl * Tp*cp_used*rho_pbl ) )**(1.0_RKIND/3.0_RKIND)
     end if
  else
     Hp69 = 29.0_RKIND * ( g0*frp_w / ( Tp*cp_used*rho_pbl ) )**(3.0_RKIND/5.0_RKIND) / U_pbl
  end if

  ! ---- B84 (Eq 18 in Sofiev et al. (2012), wildfire-adapted)
  ustar = max(0.1_RKIND, 0.1_RKIND * U_pbl)   ! proxy - could be highly uncertain
  wstar = max(0.1_RKIND, wrms)                ! proxy from resolved vertical velocity

  select case (regime)
  case (1)  ! stable
     Hp84 = 2.7_RKIND * ( g0*frp_w / ( N2_ft * U_pbl * cp_used*rho_pbl*Tp ) )**(1.0_RKIND/3.0_RKIND)
  case (2)  ! neutral
     Hp84 = 0.72_RKIND * ( g0*frp_w / ( (ustar**2) * U_pbl * cp_used*rho_pbl*Tp ) )
  case default  ! unstable
     Hp84 = 1.1_RKIND * ( g0*frp_w * (max(Habl,50.0_RKIND)**(2.0_RKIND/3.0_RKIND)) / &
                          ( (wstar**2) * U_pbl * cp_used*rho_pbl*Tp ) )**(3.0_RKIND/5.0_RKIND)
  end select
...
end subroutine plumerise_briggs

! Some functions for the above subroutines
real(RKIND) function sofiev_hp(Habl, frp_w, N2_ft, Pf0, N0sq, a, b, gexp, d)
  use mpas_kind_types
  implicit none
  real(RKIND), intent(in) :: Habl, frp_w, N2_ft, Pf0, N0sq
  real(RKIND), intent(in) :: a, b, gexp, d

! Get the plume top height: Sofiev's equation
  sofiev_hp = a*Habl + b * (max(frp_w,1.0_RKIND)/Pf0)**gexp * exp( -d * (N2_ft / N0sq) )

end function sofiev_hp

subroutine compute_N2_ft(kte, theta, z, kpbl, N2_ft)
! Estimate the Brunt-Vaisala frequency squared
  use mpas_kind_types
  implicit none
  integer, intent(in) :: kte, kpbl
  real(RKIND), intent(in) :: theta(kte), z(kte)
  real(RKIND), intent(out) :: N2_ft

  real(RKIND), parameter :: g0 = 9.81_RKIND
  integer :: k, k1, k2
  real(RKIND) :: Habl, z0, z1, zbar, thbar, num, den, slope

  Habl = max(0.0_RKIND, min(z(kpbl), z(kte)))

  ! ~2*HABL above the PBL top
  z0 = max(2.0_RKIND*Habl - 250.0_RKIND, z(kpbl) + 200.0_RKIND)
  z1 = z0 + 500.0_RKIND

  k1 = -1; k2 = -1
  do k = 2, kte
     if (k1 < 0 .and. z(k) >= z0) k1 = k
     if (k2 < 0 .and. z(k) >= z1) k2 = k
  end do
  if (k1 < 0) k1 = min(kte-2, max(kpbl+1, 2))
  if (k2 < 0) k2 = min(kte, k1+6)
  if (k2 <= k1) k2 = min(kte, k1+2)

  zbar = 0.0_RKIND; thbar = 0.0_RKIND
  do k = k1, k2
     zbar  = zbar  + z(k)
     thbar = thbar + theta(k)
  end do
  zbar  = zbar  / real(k2-k1+1, RKIND)
  thbar = thbar / real(k2-k1+1, RKIND)

  num = 0.0_RKIND; den = 0.0_RKIND
  do k = k1, k2
     num = num + (z(k)-zbar)*(theta(k)-thbar)
     den = den + (z(k)-zbar)*(z(k)-zbar)
  end do

  if (den <= 0.0_RKIND .or. thbar <= 0.0_RKIND) then
     slope = (theta(min(k1+1,kte)) - theta(k1)) / max(1.0_RKIND, (z(min(k1+1,kte)) - z(k1)))
  else
     slope = num / den
  end if

  N2_ft = (g0 / thbar) * slope

 if (N2_ft < 1.0e-6_RKIND) N2_ft = 1.0e-6_RKIND

end subroutine compute_N2_ft

subroutine mean_pbl_met(kte, u, v, w, rho, kpbl, Ubar, wrms, rhobar)
! Get the mean values within PBL
  use mpas_kind_types
  implicit none
  integer, intent(in) :: kte, kpbl
  real(RKIND), intent(in) :: u(kte), v(kte), w(kte), rho(kte)
  real(RKIND), intent(out) :: Ubar, wrms, rhobar
  integer :: k, n
  real(RKIND) :: sumU, sumW2, sumR

  sumU = 0.0_RKIND; sumW2 = 0.0_RKIND; sumR = 0.0_RKIND
  n = max(1, kpbl)

  do k = 1, kpbl
     sumU  = sumU  + sqrt(u(k)*u(k) + v(k)*v(k))
     sumW2 = sumW2 + w(k)*w(k)
     sumR  = sumR  + rho(k)
  end do

  Ubar   = sumU / real(n, RKIND)
  wrms   = sqrt(sumW2 / real(n, RKIND))
  rhobar = sumR / real(n, RKIND)
end subroutine mean_pbl_met

subroutine diagnose_regime(theta, z, w, kpbl, N2_ft, regime)
! Estimate stability regime for Briggs 
  use mpas_kind_types
  implicit none
  real(RKIND), intent(in) :: theta(:), z(:), w(:)
  integer, intent(in) :: kpbl
  real(RKIND), intent(in) :: N2_ft
  ...
  dtheta = theta(kpbl) - theta(1)

  ! stable = if strong positive stratification across the PBL
  ! unstable = if significant vertical motion in PBL and weak stratification
  ! else = neutral
  ...
  if (dtheta > 2.0_RKIND) then
     regime = 1   ! stable
  else if (wrms > 0.5_RKIND) then
     regime = 3   ! unstable
  else
     regime = 2   ! neutral
  end if
end subroutine diagnose_regime

integer function height_to_k(kte, z_lev, h)
! Get the model k-index based on height
  use mpas_kind_types
  implicit none
  integer, intent(in) :: kte
  real(RKIND), intent(in) :: z_lev(kte)
...
real(RKIND) function temperature_from_theta_pi(theta, pi, cp)
! Get temperature from theta
  use mpas_kind_types
  implicit none
